...
def check_input_mouse(pos_mouse):
    ...
    for block in list_blocks:
        ...
        if pos_left_down_x < pos_mouse_x < pos_right_down_x and pos_left_up_y < pos_mouse_y < pos_right_down_y \
            ...
            # Работа с боковыми блоками (углами)
            for block_nearby_corners in get_blocks_nearby(block):
            ...
            # Работа с боковыми блоками (крест)
            for block_nearby_cross in get_blocks_nearby(block, condition='cross'):
                if block.condition_block == ConditionBlock.Selected:
                    ...
                    if len(block_nearby_cross.list_main_blocks) == 0 and \
                            block_nearby_cross.condition_block != ConditionBlock.Selected:
...
# Получение блоков на углах поблизости
