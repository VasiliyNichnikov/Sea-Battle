...
def get_image(image, size_x=0, size_y=0):
    ready_img = pygame.image.load(image)
    if size_x != 0 and size_y != 0:
        ready_img = pygame.transform.scale(ready_img, (size_x, size_y))
    return ready_img
...
class ConditionTile(Enum):
    UP_TILE_LOCK = 0
    UP_TILE_RIGHT_LOCK = 1
    UP_TILE_LEFT_LOCK = 2
    RIGHT_TILE_LOCK = 3
    LEFT_TILE_LOCK = 4
    DOWN_TILE_LOCK = 5
    DOWN_TILE_RIGHT_LOCK = 6
    DOWN_TILE_LEFT_LOCK = 7
    SHIP = 8
    NONE = 9
...
class Block:
    def __init__(self, x, y, block_size, color_default, color_select, color_lock):
        ...
        self.condition_tile = ConditionTile.NONE
    ...
    def draw_block(self):
        tile = get_image(tile_water, size_x=block_size, size_y=block_size)
        if self.color_selected == BLACK:
            tile = get_image(tile_grass, size_x=block_size, size_y=block_size)
        elif self.color_selected == RED:
            tile = get_image(tile_test_lock, size_x=block_size, size_y=block_size)
        surface.blit(tile, (self.pos_x, self.pos_y))
...
def draw_map(first_draw=False):
    ...
    for line in range(number_blocks + 1):
        pygame.draw.line(surface, WHITE, (0, pos), (height, pos), distance_between_blocks)
        pygame.draw.line(surface, WHITE, (pos, 0), (pos, height), distance_between_blocks)
        pos += block_size
...
tile_water = 'static/img/Tiles/water_1.png'
tile_grass = 'static/img/Tiles/grass.png'
tile_test_lock = 'static/img/Tiles/test_lock.png'
